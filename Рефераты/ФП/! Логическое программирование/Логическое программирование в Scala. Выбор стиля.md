...
- какие-то вычисленные переменные могут быть вообще забыты вследствие перегрузкам функций (компилятор обычно об этом предупреждает, но всё равно пропускает),
...
\draw[->] (ScanUrl) to[bend left=20] node[above] {loadContent} (ParsingContent);
\draw[->] (ScanUrl) to[bend right=20] node[below] {obtainLimit} (ParsingContent);
