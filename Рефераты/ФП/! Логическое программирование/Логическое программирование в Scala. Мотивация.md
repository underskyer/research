...
- порядок шагов может быть перепутан, особенно когда игнорируются результаты выполнения команд; в лучшем случае это даст неоптимальные алгоритмы (если вызов `limitGetter` переместить в начало, то это вычисление окажется лишним, если в программу даже не передали адрес сайта), а в худшем неправильный порядок эффектов приведёт к неконсистентному состоянию.

Порой бывает сложно даже *обнаружить сбои*, порождённые такими ошибками.
...
- нет никаких мимолётных переменных, кроме `args` — невозможно запутаться и ошибиться;
- каждый шаг атомарен и самодостаточен — на вход принимается только результат предыдущего шага,
- следовательно, невозможно перепутать последовательность шагов.
