...
Кроме того, часто выделяют особый класс *забывающих функторов*, объединённых одной идеей. Пусть левая категория $\mathcal{C}$ состоит из тех же объектов, что и $\mathcal{D}$, но морфизмов в ней меньше — hom-объекты в $\mathcal{C}$ ограничены дополнительными условиями. Типичный пример: справа у нас категория множеств $\mathrm{Set}$ со всеми возможными функциями между ними, а слева — категория моноидов $\mathrm{Mon}$. В ней допустимы лишь те морфизмы, что переводят одни моноиды в другие. Нас тут интересует функтор $U:\; \mathrm{Mon} \rightarrow \mathrm{Set}$.
### !!! Диаграмма Обтекание!!
В категории $\mathrm{Set}$ между любыми объектами полно морфизмов, которых нет в $\mathrm{Mon}$. Основная идея заключается в том *забывающий функтор* $U$ **забывает ограничения** $\mathrm{Mon}$. Интерес представляет функтор $F$, сопряжённый к $U$ слева: $F \vdash U$.
