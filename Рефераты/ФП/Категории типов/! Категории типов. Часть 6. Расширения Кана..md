...
Ранее в [предыдущей части обзора](https://habr.com/ru/articles/970762/#scala) мы разбирали сопряжение эндофункторов в категории типов `Writer[S] ⊣ Reader[S]`. Любопытно, что для него получается очень похожий hom-изоморфизм: $\mathrm{Hom}(A \times S, B) \cong \mathrm{Hom}(A, B^S)$. Произведению объектов аналогично (с некоторой натяжкой) композиции функторов, но если первой операции противопоставляется экспоненциал, то во втором случае мы получили что-то вроде деления! Неужели эти понятия как-то связаны, или же так получилось случайно? На самом деле, случайности не случайны, и чуть позже мы убедимся в этом.
