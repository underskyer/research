...
Рассмотрим, например, сопряжение L ⊣ R. Предположим, что R не является инъективным и сводит два объекта, c и c ′ , к одному объекту d: Rc = d Rc′ = d L не имеет шансов отменить это. В то же время, он не может отобразить d одновременно к c и c ′ . Лучшее, что он может сделать, — это отобразить d к «более общему» объекту Ld, от которого идут стрелки, как к c, так и к c ′ . Эти стрелки необходимы для определения компонентов ко-единицы сопряжения: εc : Ld → c εc ′ : Ld → c ′ где Ld является, как L(Rc), так и L(Rc′ ) Ld εc ′  εc  c /d L n c ′
