...
Функторы $K$ и $F$ отображают категорию $\mathcal{C}$ в $\mathcal{A}$ и $\mathcal{D}$. Само это отображение уже как бы «сопоставляет» между собой образы функторов, формирующие подкатегории (диаграммы) $K\mathcal{C}$ и $F\mathcal{C}$. Задача функтора $K/F$ — продемонстрировать это сопоставление, «расширив» его с образа $F\mathcal{C}$ на всю категорию $\mathcal{A}$ так, чтобы естественное преобразование $\varepsilon$ явно переводило объекты образа композиции $K/F\,\circ\,F$ в образ $K$. На диаграмме эта идея отражена с посредством более широкой «трубки» $K/F$ в сравнении с желтоватой «трубкой» $F$.
