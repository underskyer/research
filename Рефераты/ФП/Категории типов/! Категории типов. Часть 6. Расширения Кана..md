...
Ранее в [предыдущей части обзора](https://habr.com/ru/articles/970762/#scala) мы разбирали сопряжение эндофункторов в категории типов `Writer[S] ⊣ Reader[S]`. Любопытно, что для него получается очень похожий hom-изоморфизм: $\mathrm{Hom}(A \times S, B) \cong \mathrm{Hom}(A, B^S)$. Произведению объектов аналогично (с некоторой натяжкой) композиция функторов, но если первой операции противопоставляется экспоненциал, то во втором случае мы получили что-то вроде деления!
