...
Монаду коплотности $F/F$ можно построить для произвольного функтора, до без дополнительных инструментов она практически бесполезна. Прежде всего, её не получится «распаковать» в отсутствие механизмов «запаковки/распаковки» функтора $F$. Также недоступен «подъём» значений и морфизмов из `F[_]` в `Codensity[F][_]`, если не предоставлено «разматрёшивание» `F`.
...
Как уже говорилось в шестой части, ключевые преимущества монады коплотности заключается в её оптимизированных операциях `fmap` и `flatten`. Первую оптимизацию мы уже рассмотрели выше — она характерна для всех расширений Кана. Вторая упоминалась в шестой части обзора — идея заключается в том, что цепочка `flatten`
