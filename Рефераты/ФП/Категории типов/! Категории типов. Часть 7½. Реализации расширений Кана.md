...
В шестой части обзора уже [упоминались](https://habr.com/ru/articles/1012402/#yoneda) представления Йонеды функтора, как его расширения Кана вдоль тождественного $Id$. В частности:
```scala
...
def  lift[F[+_]: Functor]:        F  ~> Yoneda[F] = uRan[F, Id, F](id)  
def lower[F[+_]         ]: Yoneda[F] ~>        F  = εRan[F, Id]
//  lift[F] ⋅ lower[F] = id[F]
// lower[F] ⋅  lift[F] = id[Yoneda[F]]
```
