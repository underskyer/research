...
Обратите внимание: механизм продолжений, заложенный в представление ко-Йонеды позволил упростить квадратичную сложность монадной композиции во `Free` до линейной во `Freer`.

И всё же в современных библиотеках предпочитают более ленивые реализации свободной монады, нацеленные на интроспекцию и оптимизацию полной цепочки вычислений.
...
Давайте снова перечислим определяющие свойства свободной монады функтора $F$. Прежде всего, это *функтор* $T$, дополненный двумя монадными преобразованиями, а также преобразованием «подъёма» из $F$. Все эти преобразования приводят в $T$ и осталось только разобраться с функториальностью $(a \to b) \to (Fa \to Fb)$. Для этого применим де**карри**рованием и получим изоморфизм ниндзя-ко-Йонеды: $\int^x Fx \times \mathrm{Hom}(x, a) \cong Fa$. В результате получим такие четыре преобразования:
$$
\begin{split}
...
\\
coyo & \colon & Id \backslash T \rightsquigarrow T.
\end{split}
$$
Мы можем объединить их в единое преобразование, и так как оно *полностью определяет* свободную монаду, то на самом деле мы получим именно изоморфизм:
$$
T \cong Id + F + T \circ T + Id \backslash T.
$$
