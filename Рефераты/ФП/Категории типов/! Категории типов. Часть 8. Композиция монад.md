...
Итак, не всякий функтор может стать монадой даже теоретически. Даже если у нас есть пара монад $F$ и $G$, то композитный функтор $F \circ G$ не обязан быть монадой. [Ранее](https://habr.com/ru/articles/957848/#distributive) мы уже касались этой проблемы. В частности, тогда говорилось, что для сборки такой монады необходим *дистрибутивный закон* — преобразование $\lambda \colon G \circ F \rightsquigarrow F \circ G$, удовлетворяющее определённым законам. Проблема композиции монад заключается в том, что не для всяких двух $F$ и $G$ дистрибутивный закон в вообще существует. Для программистов это означает, что не все эффекты можно переставить местами и не для всех комопзитных эффектов можно строить цепочки вычислений.

И тут на самом деле нет причин огорчаться, потому что такая модель точно отражает действительность. Наглядный пример — комбинирование эффектов журналирования и недетрминированности (манипулирование колекцией). Вычисление цепочки таких эффективных преобразований будет давать разные итоговые журналы в зависимости от того, будем ли мы сперва применять все преобразования для каждого элемента коллекции, и лишь затем собирать журнал, или же мы будем пополнять журнал для преобразования каждого элемента.

### !!! СПОЙЛЕР `Writer[String] ∘ List`
В качестве примера рассмотрим комопзитный функтор `Writer[String] ∘ List`:
```scala
type Writer[S] = [A] =>> S × A  
...
```
Для строки задан естественный моноид вокруг операции конкатенации. Но проблема в том, что именно для строки эта операция не будет коммутативной! По этой причине в следующем примере разный порядок вычисления `flatMap` даст неодинаковый результат:
```scala
