...
Итак, не всякий функтор может стать монадой даже теоретически. Даже если у нас есть пара монад $F$ и $G$, то композитный функтор $F \circ G$ не обязан быть монадой. [Ранее](https://habr.com/ru/articles/957848/#distributive) уже говорилось, для сборки такой монады необходим *дистрибутивный закон* — преобразование $\lambda \colon G \circ F \rightsquigarrow F \circ G$, удовлетворяющее определённым законам. Проблема композиции монад заключается в том, что не для всяких двух $F$ и $G$ в вообще существует дистрибутивный закон. Для программистов это означает, что не все эффекты можно переставить местами и не для всех комопзитных эффектов можно строить цепочки вычислений.

```scala
type Writer[S] = [A] =>> S × A  
type M = Writer[String] ∘ List // комопзитный функтор - монада ли?
...
```
Разный порядок композиции приводит к разным результатам, что недопустимо для монады. На самом деле никакая реализация `flatMap` не позволит получить одинаковые значения в подобных тестах.
