...
Рассмотрим дистрибутивный закон $\lambda^G(F)\colon F \circ G \rightsquigarrow G \circ F$ для *фиксированного* функтора $G$ и произвольного $F$. Подставляя вместо $F$ различные варианты (экспоненциал, произведение, константный функтор в терминальный объект…) можно убедиться, что функтор $G$ *обязан* сохранять вообще любые пределы. [Ранее](https://habr.com/ru/articles/970762/#continuty) уже говорилось, что такие функторы называются *непрерывными*. Согласно [теореме Фрейда](https://bartoszmilewski.com/2020/07/22/freyds-adjoint-functor-theorem/) в локально представимых категориях (именно с такими работают программисты) непрерывные функторы являются *представимыми*, то есть $Ga \cong \mathrm{Hom}(r, a)$ для некоторого объекта $r$, который и представляет весь этот функтор.
