...
Рассмотрим дистрибутивный закон $\lambda^G(F)\colon F \circ G \rightsquigarrow G \circ F$ для *фиксированного* функтора $G$ и произвольного $F$. Подставляя вместо $F$ различные варианты (экспоненциал, произведение, константный функтор в терминальный объект…) можно убедиться, что функтор $G$ *обязан* сохранять любые пределы. [Ранее](https://habr.com/ru/articles/970762/#continuty) уже говорилось, что такие функторы называются *непрерывными*.

Существует специальная [теорема Фрейда о представимости](https://math.berkeley.edu/~willfisher/papers/Category_Theory_Notes.pdf#12#2), согласно которой любой непрерывный функтор $G$ в «достаточно хорошей» категории (именно с такими работают программисты) являются *представимыми*, то есть $Ga \cong \mathrm{Hom}(r, a)$ для некоторого объекта $r$, который и представляет весь этот функтор.

$$
\begin{split}
F \circ G \rightsquigarrow G \circ F
\\
G = r \to \_ \equiv \mathrm{Hom}(r,\_)
\\
F(\mathrm{Hom}(r,a)) \cong \mathrm{Hom}(r,Fa)
\\
