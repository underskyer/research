...
По этой причине монадные трансформеры стараются выносить в самостоятельные типы данных, чтобы не мешать выводу типов:
...
Помимо конечных коллекций дистрибутивными будут и непустые бесконечные рекурсивные структуры данных. В [обзоре рекурсивных типов](https://habr.com/ru/articles/863376/#rec_derivatives) показывалось, как их производные позволяют фокусироваться на конкретных ячейках хранения значения. Значит можно предоставить тип, значения которого будут индексировать ячейки контейнера — это и будет его «представитель».
...
Тогда мы можем реализовать монадный трансоформер:
```scala
...
- Композиция монад
	- Публикации в научных журналах
		- [Combining Monads](https://link.springer.com/chapter/10.1007/978-1-4471-3215-8_12) оригинальная статья 1992 года Дэвида Кинга и Филипа Уодлера
...
- Distributive
	...
	- [Every Distributive is Representable](https://duplode.github.io/posts/every-distributive-is-representable.html) пост Дэниела Млота в его блоге
	...
	- [Representable Functors](https://bartoszmilewski.com/2015/07/29/representable-functors/) пост в Кафе программирования Бартоша Милевски
- Traversable
...
Оба класса типов не покрывают все возможные контейнеры и автор статьи [Пытаясь композировать некомпозируемое: монады](https://habr.com/ru/articles/536088/?code=dc540f3d8ccdce2dd52fe97d96a85b0a&state=ECBwfm3OeaYAG7jGzsD5exJi&hl=ru) разбирает частные случаи устройства монад как композиций сопряжённых функторов, что даёт и другие полезные монадные трансформеры.

Конечно же, для композиции функторов мы всегда можем использовать свободные монады `Free[F ∘ G]`, рассмотренные в [предыдущей части обзора](https://habr.com/ru/articles/1044512/). Однако такую «свободную программу» в конце придётся интерпретировать в контейнер `M[_]`, и либо он будет у́же композиции и растеряет все эффекты, либо же всё сохранит, но останется «нечестной» монадой, нарушающей законы.
