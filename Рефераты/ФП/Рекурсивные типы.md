...
Теория вычислимости берёт своё начало с работы Алана Тьюринга [On Computable Numbers, With An Application to Entscheidungsproblem](https://www.cs.virginia.edu/~robins/Turing_Paper_1936.pdf). В частности, он сформулировал понятие *примитивной рекурсивной функции*. Все такие функции замкнуты относительно обычной композиции функций и специального комбинатора примитивной рекурсии $\eta$. Например, если $\phi: (Nat, A) \Rightarrow A$ – вычислимая функция, а $r: A$, то также будет вычислимой следующая функция:
$$
\begin{split}
\eta : Nat &\Rightarrow A\\
\eta(0) &= r\\
\eta(n) &= \phi(n,\;\eta(n-1))
\end{split}
$$
В последней строчке видна суть рекурсии – в *теле* функции есть вызов *этой самой функции*. Можно сказать, что в процессе вычисления этой функции, ещё раз потребуется начать вычисление этой функции (но, уже с другими параметрами), в которой снова… ну, вы поняли 😉. Примитивно рекурсивные функции от целочисленного аргумента отражают идею математической индукции – если у нас есть значения $\eta(0)$ и если, имея $\eta(n)$ мы можем вычислить $\eta(n+1)$, то мы *всегда* можем вычислить $\eta(n)$ для любого (конечного!) натурального $n$.

Однако не для всех алгоритмов 
