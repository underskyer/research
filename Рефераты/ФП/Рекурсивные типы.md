...
Теория вычислимости берёт своё начало с работы Алана Тьюринга [On Computable Numbers, With An Application to Entscheidungsproblem](https://www.cs.virginia.edu/~robins/Turing_Paper_1936.pdf). В частности, он сформулировал понятие *примитивной рекурсивной функции*. Все такие функции замкнуты относительно обычной композиции функций и специального комбинатора примитивной рекурсии $\eta$. Например, если задано число $r$ и функция двух аргументов $\phi$, то то оператор $\eta$ определяется так:
$$
\begin{split}
\eta(0) &= r\\
\eta(n) &= \phi(n,\;\eta(n-1))
...
Однако, не все полезные алгоритмы можно записать с помощью примитивной рекурсии. Тьюринг вводит также класс *частично определённых рекурсивных функций*. Такое определение означает, что для некоторых значений аргумента функция становится зацикленной или *расходящейся* (что в реальном мире соответствует сбою, ошибке). Класс частично определённых рекурсивных функций вводится аналогично примитивным, но дополняется *оператором минимизации аргумента* $h(f)$, который вычисляет *минимальное* значение аргумента $f(y)$, обнуляющее $f$:
$$
\begin{split}
f(y) &= 0\\
h(f) &= \min y\\
...
\end{split}
$$
Оператор $h$ не всегда корректно определён, поскольку функция $f$ может быть не равной нулю ни при каких значениях аргументов.

> Отдельного упоминания заслуживает понятие *общерекурсивных функций* – это подкласс частично определённых рекурсивных функций, но возвращающих значения для *любого* своего аргумента.

Он ввел понятие [абстрактной вычислительной машины](https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D1%88%D0%B8%D0%BD%D0%B0_%D0%A2%D1%8C%D1%8E%D1%80%D0%B8%D0%BD%D0%B3%D0%B0), получившей впоследствии его имя, и доказал фундаментальную теорему о неразрешимости задачи о её остановке. 

Алан Тьюринг разработал 
