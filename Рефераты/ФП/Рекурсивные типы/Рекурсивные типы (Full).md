...
> В [предыдущей статье рассказывалось](https://habr.com/ru/articles/807495/#continuations), как в ФП-шном "стиле передачи продолжений" можно написать своеобразные аналоги `goto`. Фокус в том, что получаемые решения применимы лишь в узком наборе сценариев, где использование `goto` *автоматически* оказывается безопасным. И, конечно же, “переход назад” в CPS-стиле так или иначе реализуется посредством рекурсии.
...
```
В реализации `fib` видно преимущество новой сигнатуры – «обратную ссылку» `self` можно композировать с собой же.
...
Покажем, что `μ[F]` является неподвижной точкой для `F[_]`, то есть, что типы `μ[F]` и `F[μ[F]]` изоморфны. Для этого достаточно предоставить такие реализации `Algebra[F][μ[F]]` и `Coalgebra[F][μ[F]]`, чтобы их комбинации давали тождественную функцию. Ещё придётся явно предоставить доказательство [ковариантности](https://habr.com/ru/articles/807467/#variance) `F[+_]` – сделаем это посредством класса типов `Lift` (см. [предыдущую статью](https://habr.com/ru/articles/807485/#container_typeclasses)):
```scala
type Lift[F[_]] = [A, B] => (A => B) => (F[A] => F[B])

def inμ [F[_]: Lift]: F[μ[F]] =>   μ[F]  = fμF => [X] => (alg: Algebra[F][X]) => alg(summon[Lift[F]](foldμ(alg))(fμF))
def outμ[F[_]: Lift]:   μ[F]  => F[μ[F]] =  μF => foldμ(summon[Lift[F]](inμ))(μF)
```
...
```scala
def unfold[F[_]]: [X] => LazyFx[F][X] => 𝛎[F] =
  [X] => (lazyFx: LazyFx[F][X]) =>
...
```scala
def out𝛎[F[_]: Lift]:   𝛎[F]  => F[𝛎[F]] =  𝛎F => summon[Lift[F]]((x: 𝛎F.T) => unfold((x, 𝛎F.value._2)))(eval(𝛎F.value))
def  in𝛎[F[_]: Lift]: F[𝛎[F]] =>   𝛎[F]  = f𝛎F => unfold((f𝛎F, summon[Lift[F]](out𝛎))) // косвенная рекурсия 😉
...
val factorial10: 𝛎List[Int] =
  unfold[OptCell[Int]](          // развёртка
    (0, 1),                      // начальное состояние (0! = 1)
    {(n, res) =>                 // функция преобразования состояния
      Option.when(n <= 10)((     // ограничение на размер (может отсутствовать!)
        res,                     // промежуточный результат
        (n + 1, (res * (n + 1))) // состояние в следующей итерации
      ))
    }
  )
```
...
def fold𝛎[F[_]: Lift, X](alg: Algebra[F][X]): 𝛎[F] => X =
  𝛎F => alg(summon[Lift[F]](fold𝛎(alg))(out𝛎(𝛎F)))
...
val factorial_∞ : 𝛎List[Int] =
  unfold[OptCell[Int]](
    (0, 1),
    {(n, fact) => Some(( // вместо `Option.when(n <= 10)`
      fact,
      (n + 1, (fact * (n + 1)))
    ))}
  )
...
def обратно: μ[F] => 𝛎[F] = μF => unfold((μF, outμ))
```
...
```scala
type   Fold = [Fix[_[_]]] =>> [F[_], X] => Lift[F] ?=>   Algebra[F][X] => Fix[F] =>     X
type Unfold = [Fix[_[_]]] =>> [F[_], X] => Lift[F] ?=> Coalgebra[F][X] =>     X  => Fix[F]

def   fold[Fix[_[_]]:   Fold, F[_]: Lift, X] = summon[  Fold[Fix]][F, X]
def unfold[Fix[_[_]]: Unfold, F[_]: Lift, X] = summon[Unfold[Fix]][F, X]
```
...
##### МОЖНО ЛИ СОБРАТЬ FOLD ИЗ IN И OUT???
