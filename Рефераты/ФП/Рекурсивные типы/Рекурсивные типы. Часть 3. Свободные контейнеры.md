...
```
Может показаться, что вся магия оптимизации хвостовой рекурсии заключается здесь в наличии возможности `flatMap`. Но гораздо важнее наличия, оказывается «ленивость» конкретной реализации `flatMap`, которая во многом определяется структурой контейнерного типа.
