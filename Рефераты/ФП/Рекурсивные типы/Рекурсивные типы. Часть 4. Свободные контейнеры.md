...
```
Фокус в том, что `tailRecM` из `cats.Monad[Id]` реализован неуниверсально, а специфично для `Id`, через оптимизацию хвостовой рекурсии (и аннотацию `@tailrec`). Впрочем аналогичным способом в библиотеке реализуются `tailRecM` и для других стандартных контейнеров.
...
```
Метод зацикливает указанный эффект, что бывает полезно при организации действий через какие-то промежутки времени в течении работы приложения.
...
- [Histomorphism, Dynamorphism, Futumorphism](https://free.cofree.io/2017/11/13/recursion/#histomorphism)
- [Recursion Schemes, Part IV: Time is of the Essence](http://blog.sumtypeofway.com/recursion-schemes-part-iv-time-is-of-the-essence/) в блоге Патрика Томпсона
