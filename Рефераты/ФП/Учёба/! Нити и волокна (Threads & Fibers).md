...
В языке Go предложили способ избежать этой проблемы, полностью отказавшись от обобщённых контейнеров для управления асинхронностью. Даже без явных кооперативных пометок вроде `await` *среда исполнения сама находит блокирующие операции* и приостанавливает выполнение задачи, отправляя её продолжение в очередь. Нити операционной системы и ядра процессора при этом не простаивают, а программисты могут писать код в привычном стиле, не задумываясь о «цвете» функций. Такой механизм языка Go получил название **горутины** (goroutines).
