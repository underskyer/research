...
Вообще-то уже горутины с виртуальными нитями Loom принято называть «зелёными нитями» (green threads) и даже «**волокнами**» (fibers). Но в Scala в экосистемах Cats Effect (CE) и ZIO представлены одноимённые сущности, которые представляют гораздо больше возможностей чем корутины. Поэтому закрепим термин «волокна» именно за этими сущностями.
...
В этом случае асинхронные функции имеют вид `A => IO[B]`, что очень похоже на рассмотренные ранее корутины, основанные на контейнере `Future[_]`. Но у контейнера `IO[_]` есть существенное отличие, дающее максимальный контроль над операциями ввода-вывода. Дело в том, что в соответствии с парадигмой функционального программирования все операции по «запаковке» в `IO[_]` и преобразованиях в нём являются *чистыми, не вызывающими побочных эффектов*. В контейнере хранятся лишь данные — *план вычислений*, который выполняется лишь при «распаковке», неявно производимой библиотечным интерпретатором в самом конце программы. ФП-шная чистота функций, [ссылочная прозрачность](https://ru.wikipedia.org/wiki/%D0%A1%D1%81%D1%8B%D0%BB%D0%BE%D1%87%D0%BD%D0%B0%D1%8F_%D0%BF%D1%80%D0%BE%D0%B7%D1%80%D0%B0%D1%87%D0%BD%D0%BE%D1%81%D1%82%D1%8C) позволяет вносить любые коррективы в план вычислений до его исполнения — добавить финализаторы, которые интерпретатор гарантированно запустит по завершению задачи, настроить повторения, связать с параллельными задачами и т.п.

В противоположность этому, `Future` «жадно» запускает вычисления в момент создания. При этом, мы не сможем запустить их повторно, или по желанию приостановить вычисления, так нет гарантий, что в логике корректно реализованы механизмы кооперативной отмены. Волокна CE/ZIO не полагаются на кооперативность — интерпретатор на любой `flatMap` (и не только) может прерывает волокно на основании флага отмены, предварительно запустив пользовательскую финализацию.
...
1. Волокно как структура данных (Blueprint)

- **Тезис:** В Scala `IO` — это не запущенная задача (как `Future` или `Task`), а **описание** вычислений.
- **Суть:** Это позволяет интерпретатору (Runtime) полностью контролировать процесс выполнения. Волокно здесь — это логический поток, который рантайн может «разобрать» и «собрать» заново.

2. Автоматические точки прерывания (Safe Points)

- **Тезис:** Проверка флага отмены встроена в саму механику композиции.
- **Суть:** Каждый вызов `flatMap` в коде на ZIO/CE является «безопасной точкой». Интерпретатор проверяет статус волокна перед выполнением следующего шага. Если волокно отменено, выполнение просто не переходит к следующему шагу цепочки. **Программисту не нужно писать `if (cancelled)` вручную.**

3. Алгебра ресурсов (Bracket / Scope)

- **Тезис:** Гарантированная финализация (очистка).
- **Суть:** Поскольку отмена встроена в рантайн, он «знает», какие ресурсы были захвачены. Конструкции типа `acquireRelease` (в ZIO) или `bracket` (в Cats Effect) гарантируют, что код закрытия файла или соединения будет выполнен **всегда**, даже если волокно было прервано «на лету». Это то, что невозможно сделать в императивном стиле без риска утечек.

4. Семантика прерывания (Interruption)

- **Тезис:** Прерывание — это не исключение, а сигнал рантайму.
- **Суть:** В отличие от Java `InterruptedException`, который можно случайно «проглотить» в `catch`, прерывание в ZIO/CE нельзя игнорировать на уровне бизнес-логики. Оно обрабатывается самим интерпретатором «над» вашим кодом.

5. Структурный параллелизм (Structured Concurrency) из коробки

- **Тезис:** Жизненный цикл дочерних задач жестко привязан к родителю.
- **Суть:** Если вы запускаете две задачи параллельно (`zipPar`, `race`) и одна из них падает или завершается первой, вторая будет **автоматически и немедленно отменена** рантаймом. Это предотвращает появление тех самых «зомби-задач» без каких-либо усилий со стороны разработчика.

---

Подсказка для финальной мысли:

Вы можете завершить статью тем, что Scala-подход — это **«безопасность по умолчанию»**.
...
**Будет очень круто, если вы закончите на противопоставлении: «Удобство для пишущего (Loom/Go) против Уверенности в работающем (ZIO/CE)».**
