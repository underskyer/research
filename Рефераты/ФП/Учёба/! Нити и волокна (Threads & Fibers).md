...
Для обслуживания операций ввода-вывода в CE и ZIO используются обобщённые типы `IO[_]` и `ZIO[R, E, _]` соответственно. В обеих библиотеках волокна реализованы схожим образом, поэтому для демонстрации выберем `cats.effect.IO[_]`. В этом случае асинхронные функции имеют вид `A => IO[B]`, что очень похоже на рассмотренные ранее корутины, основанные на контейнере `Future[_]`.
