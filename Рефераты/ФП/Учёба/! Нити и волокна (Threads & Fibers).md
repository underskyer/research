...
- Процессор компьютера распределят исполнение алгоритмов по большому количеству других устройств. Вычисление на каждом устройстве занимает определённое время и в наивном подходе *процессор обязан ожидать, впустую растрачивая собственный ресурс*.
- Современные операционные системы реализуют механизм **вытесняющей многозадачности**, распределяя многочисленные задачи от совместно запущенных процессов на **нити** (threads), с определённой периодичностью принудительно переключая исполнение этих нитей на ядрах процессора.
- Нити являются дорогостоящим ресурсом, рациональное использование которого доверяется приложениям. Обычно приложения ещё на старте запрашивают у операционной системы ограниченный **пул потоков** (нитей) и распределяют свои задачи уже в рамках этого пула. Однако, гарантии безопасности отсутствуют, поэтому не редко случаются **утечки нитей**, что приводит к остановке приложения и негативно сказывается на работе системы в целом.
- Механизм управления нитями универсален и поэтому очень громоздок. Под высокой нагрузкой возникает критическая проблема **пробуксовки**, когда управление нитями отнимает больше вычислительных ресурсов, чем полезная логика приложений.
- Простым решением является отказ от дополнительных нитей. Алгоритмы разбиваются на небольшие фрагменты, разделённые этапами «ожиданий», и эти фрагменты складываются в очередь задач, которая пробегается в простом **цикле событий** в единственной нити (или в небольшом пуле, соразмерным с количеством ядер). Такое решение многократно поднимает максимальный RPS веб-сервера в сравнении со стратегией, основанной на переключении нитей.
- Наивная реализация техники цикла событий приводит к разбиению алгоритмов на слабо связанные блоки-продолжения. Это порождает **ад обратных вызовов**, радикально усложняющий отладку и обработку исключений.
- Снова собрать все фрагменты алгоритма в целостную последовательность позволяет техника, известная под общим названием «**корутины**» (сопрограммы, асинхронные методы). Обычно она реализуется через синтаксический сахар или монадическую композицию. Вызов методов вроде `flatMap` или конструкции `await` выступают в роли маркеров для **кооперативной многозадачности**. Это места где компилятор или среда исполнения *сами* декомпозируют алгоритм на фрагменты, между которыми можно приостановить выполнение одной задачи и переключиться на другую. Корутины упрощают отладку и обработку исключений.
...
- Но дело в том, что императивные техники программирования концептуально не способны предоставить программистам удобные инструменты контроля над процессом вычислений. В частности, актуальными остаются проблемы безопасной отмены задачи и связанные с этим потенциальные утечки.
- Эти проблемы можно решить только декларативными техниками функционального программирования. В Scala-библиотеках CE и ZIO используются «ленивые контейнеры», которые не просто являются надстройкой на исполняемой задачей, *накапливают план вычислений*, позволяя детально настроить его ещё до начала исполнения. Это позволило реализовать *безопасный* механизм отмены задач в моментах монадической композиции, с гарантированным запуском пользовательских финализаторов. Такие корутины называются **волокнами** (fibers). Они дают много интересных возможностей по управлению параллелизмом, одной из которых является *безопасная автоматическая отмена* дочерних волокон при отмене родительской. Такой структурированный параллелизм обеспечивает максимальную защиту от утечек.
