...
- **Многопоточность** — способ реализации многозадачности внутри одного процесса, при котором выполнение разделяется на несколько потоков (нитей), использующих общую память.
- **(Псевдо)Параллельность** — физическое выполнение нескольких операций в один и тот же момент времени на разных ядрах процессора (или создание такой иллюзии за счет сверхбыстрого переключения контекста).
...
- **Асинхронность** — стратегия программирования, позволяющая запустить длительную операцию без блокировки текущего потока исполнения, чтобы вернуть ресурс системе и возобновить работу только при готовности результата.
