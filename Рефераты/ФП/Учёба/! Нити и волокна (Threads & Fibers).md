![[Pasted image 20260304182726.png]]
...
- **Многозадачность** — способность операционной системы или среды исполнения управлять выполнением нескольких процессов, переключая между ними ресурсы компьютера.
...
- **Параллельность** — выполнение нескольких вычислений в один и тот же период времени. Часто разделяют физическую параллельность (одновременное выполнение на нескольких ядрах) и *псевдо*параллельность (быстрое переключение задач). Но на самом деле такие экивоки сильно переоценены. Параллельно — значит параллельно!
...
- **Асинхронность** — стратегия программирования, позволяющая запустить длительную последовательность операций, не блокируя поток исполнения на время ожидания промежуточных результатов, а приостанавливая её, освобождая вычислительные ресурсы для других задач.
