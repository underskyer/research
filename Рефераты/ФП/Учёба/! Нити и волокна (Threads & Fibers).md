...
В противоположность этому `Future` «жадно» запускает вычисления в момент создания. Мы не сможем запустить их повторно или по желанию приостановить, так как нет гарантий, что в логике корректно реализованы механизмы кооперативной отмены. Волокна CE/ZIO не полагаются на кооперативность: интерпретатор на любой `flatMap` (и не только) может *прервать* волокно на основании флага отмены, предварительно запустив пользовательскую финализацию. Кроме того, при необходимости подготовленный план вычислений можно дополнить любой стратегией перезапуска.
...
def longTask: IO[Unit] =  
  IO.println("Работа началась...")  
    ...
    .onCancel(IO.println("Задача отменена, чистим ресурсы...")) // финализатор!  
...
// Работа началась...
// Решили отменить...
// Задача отменена, чистим ресурсы...
```
