...
# Императивные отменяемые волокна?? (direct style)
...
```
Да, это знакомый синтаксический сахар `async/await`, существующий в C# и других языках. На этапе компиляции такой код декомпозируется и соединяется во всю ту же цепочку `flatMap`.
