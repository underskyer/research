...
def structuralParallelism =  
  ...
  yield ()
```
Такая техника называется «**структурированный параллелизм**». Она даёт максимальную защиту от утечки волокон. Конечно, если очень постараться, то для каки-то целей можно организовать такую «утечку» вручную, но получить её случайно практически невозможно.
